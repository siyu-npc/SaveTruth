...
"list"==L?K[M].innerHTML=r.tmpl(p,D):"banner"==L?K[M].innerHTML=r.tmpl(d,D):"card"==L&&(K[M].innerHTML=r.tmpl(s,D));
}
}catch(W){
console.log(W);
}
...
var c=1e3,m=window.user_uin||0,l=0!==m&&Math.floor(m/100)%1e3<c,_=(i>=43305||d>=16)&&o.isGif(),g=0!==m&&Math.floor(m/100)%1e3<=100,u=d>=564&&o.canHevc()&&h.gtVersion("6.8.0",!0)&&g;
...
return T&&(y||A)&&(window.__addIdKeyReport("28307",106),o=J.addParam(o,"tp","wxpic",!0),
...
var d=t[a];
d&&d.tagName&&"iframe"!=d.tagName.toLowerCase()?n(d):d&&d.tagName&&"iframe"==d.tagName.toLowerCase()&&"video_ad_iframe"===d.getAttribute("class")&&d.setAttribute("class","");
}
}catch(s){}
}(),function(){
