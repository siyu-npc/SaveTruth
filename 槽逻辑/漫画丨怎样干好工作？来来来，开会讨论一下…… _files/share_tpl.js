...
var k=f.app_info.down_count||0,I=f.app_info.app_size||0,S=f.app_info.app_name||"",q=f.app_info.category,z=["万","百万","亿"];
if(k>=1e4){
k/=1e4;
for(var D=0;k>=10&&2>D;)k/=100,D++;
k=k.toFixed(1)+z[D]+"次";
}else k=k.toFixed(1)+"次";
I=formSize(I),q=q?q[0]||"其他":"其他",S=formName(S),f.app_info._down_count=k,f.app_info._app_size=I,
f.app_info._category=q,f.app_info.app_name=S,u={
appname:f.app_info.app_name,
...
var E=f.card_info.card_id||"",O=f.card_info.card_ext||"";
O=O.htmlDecode();
try{
O=JSON.parse(O),O.outer_str=f.card_info.card_outer_id||"",O=JSON.stringify(O);
}catch(C){
O="{}";
}
u={
...
for(var P in R)if(0==P.indexOf("pos_")){
var u=R[P],f=!!u&&u.a_info;
if(u&&f){
if(2!=m){
if(0==f.pos_type){
if(f.new_appmsg=window.new_appmsg,js_bottom_ad_area.innerHTML=TMPL.tmpl(a_tpl,f),
111==f.pt||112==f.pt||113==f.pt||114==f.pt){
var H=document.getElementsByClassName("js_download_app_card")[0],U=H.offsetWidth,K=Math.floor(U/2.875);
...
108==f.pt||109==f.pt||110==f.pt?document.getElementById("js_main_img").style.height=L/1.77+"px":116==f.pt||117==f.pt,
...
J=!0;else{
var W=require("a/tpl/cpc_tpl.html.js"),G=f.exp_obj.sale_text;
(23==f.product_type||12==f.product_type||19==f.product_type||46==f.product_type)&&(G=f.avatarTitle),
js_cpc_area.innerHTML=TMPL.tmpl(W,{
...
(12==f.product_type||19==f.product_type)&&(J=!0,f.app_info&&f.app_info.icon_url&&(f.avatar=f.app_info.icon_url,
f.avatarTitle=f.app_info.appname)),46==f.product_type&&f.game_info&&f.game_info.head_img&&(f.avatar=f.game_info.head_img,
f.avatarTitle=f.game_info.nick_name),4==f.pos_type&&_checkShowCpc()){
f=_parseExpCpc(f);
...
avatar:f.avatar
},4==f.pos_type&&(J&&(V.customUpdataFunc=function(e,a){
...
}),ad_render_object["pos_"+f.pos_type]=new CrtManager.createCrtObject(X,F,js_cpc_area,V),
